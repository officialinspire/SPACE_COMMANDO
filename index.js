...
(() => {
  ...
  // Additional animation definitions for crouching.  The sprite
  // sheet does not provide separate crouch frames, so we reuse
  // existing ones while still cropping in the draw routine.  Having
  // explicit keys allows the draw logic to select frames based on
  // intent (idle, walking or shooting while ducking) rather than
  // falling back to the standing animations.  Each entry matches
  // corresponding run/shoot frames.
  ANIMATIONS.playerCrouch = [ { sx:0, sy:0 } ];
  ANIMATIONS.playerCrouchWalk = [ { sx:0, sy:0 }, { sx:32, sy:0 }, { sx:64, sy:0 }, { sx:96, sy:0 } ];
  ANIMATIONS.playerCrouchShoot = [ { sx:128, sy:0 }, { sx:160, sy:0 }, { sx:192, sy:0 } ];
  ...
  try { jumpSfx.currentTime = 0; jumpSfx.play(); } catch (err) {}
    ...
    function drawGame() {
      ...
      if (sheetLoaded) {
        ...
        let frames;
        // Determine if the player just fired.  isShooting is true
        // whenever the shoot key is held and the shot cooldown is
        // positive, which corresponds to the firing animation window.
        const isShooting = (keys['z'] || keys['Z']) && player.shootCooldown > 0;
        if (!player.onGround) {
          // Jumping takes precedence regardless of ducking state.
          frames = ANIMATIONS.playerJump;
        } else if (player.isDucking) {
          // When crouching select specialised frames for each action.
          if (isShooting) {
